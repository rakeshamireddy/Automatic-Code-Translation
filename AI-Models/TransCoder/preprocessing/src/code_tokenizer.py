...
#from pyper import R # Added for R
...
R_TOKEN2CHAR =  PYTHON_TOKEN2CHAR.copy() # Added for R
R_CHAR2TOKEN =  PYTHON_CHAR2TOKEN.copy() # Added for R                   
...
def tokenize_r(s, keep_comments=False):
    try:
        ...
        try:
            ...
            #r = R()
            #iterator  = r("source( Tokenization.R )") # Added for R
...
def extract_functions_r(s):
    ...
    while True:
        #print("Entered while")
        try:
           # print("Entered try 1") # <- function()
            if token == 'function': # Modified for R
               # print("token 1")
                function = ['function']
               # print("function1", function )
                while not (token == '}'):
                #while not (token == 'DEDENT' and number_indent == 0): # Commented for R
                     token = next(tokens)
                     if token == 'INDENT':
                         number_indent += 1
                     elif token == 'DEDENT':
...
def get_function_name_r(s):
    assert isinstance(s, str) or isinstance(s, list)
    if isinstance(s, str):
        s = s.split()
    return s[s.index('function') + 1] # Modified for R
