...
def load_binarized(path, params):
    ...
    #print('debug_train:',params.debug_train) commented by Gayathri Ganesh
    if params.debug_train:
        path = path.replace('train', 'valid')
    if getattr(params, 'multi_gpu', False):
        print('params.split_data_accross_gpu:',params.split_data_accross_gpu)
        print('params.local_rank:',params.local_rank)
        print('params.global_rank:',params.global_rank)
        print('path[:-4]:',path[:-4])
        ...
        print('params.split_data:',params.split_data)
...
def check_data_params(params):
    ...
    #assert all([l1 in params.langs and l2 in params.langs for l1,
    #            l2 in params.bt_steps])        # Modified       
    assert all([l1 == l3 and l1 != l2 for l1, l2, l3 in params.bt_steps]) 
    #assert all([l1 != l2 for l1, l2 in params.bt_steps]) # Modified
     ...
     #   lang: {
      #      [splt: os.path.join(params.data_path, '%s.%s.pth' % (splt, lang))
       #     for splt in ['valid', 'test']],
        #    [splt: os.path.join(params.data_path, '%s.%s.0.pth' % (splt, lang))
         #   for splt in ['train']]
       # } for lang in params.langs if lang in required_mono
   # }
    ...
    print('params.mono:',params.mono_dataset.values())
    ...
    print('params.clm_steps:',params.clm_steps)
    print('params.mlm_steps:',params.mlm_steps)
    print('params.mt_steps:',params.mt_steps)
    required_para_train = set(
        params.clm_steps + params.mlm_steps + params.mt_steps)
    ...
    print('para_dataset:',params.para_dataset) 
